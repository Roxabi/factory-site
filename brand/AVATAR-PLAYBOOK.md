...
> **Palette note (brand v2 rollout, 2026-04-20):** the `#0a0a0f` Obsidian background and `#e85d04` Forge Orange rim are **v1 marketing-brand** references and stay that way for avatar photography. Bot profile pictures are marketing surfaces — they live on the v1 aesthetic, not the v2 GitHub-dark `#0d1117` surface used in docs/dep-graphs. Do not "modernize" these hex values when updating avatars. See `BRAND-BOOK.md § 6` for the v1-vs-v2 surface-selection rule.
...
uv run imagecli batch ~/.roxabi/forge/lyra/brand/prompts/avatar-<agent>/ \
  --output-dir ~/.roxabi/forge/lyra/brand/concepts/avatar-<agent>/
```
...
All brand galleries are served by the forge dev server — never opened as `file://`.

### Server setup

```bash
# From ~/projects/ — start/stop/logs via the supervisor hub
make forge start
make forge logs    # tail stdout
make forge stop
```

The server at `~/.roxabi/forge/serve.py`:
- Serves static files from `~/.roxabi/forge/`
- Exposes `/api/list/<path>` — returns a JSON listing of files in that directory
- Pushes SSE live-reload events on file change

Brand assets live natively at `~/.roxabi/forge/lyra/brand/`, so `http://localhost:8080/lyra/brand/` maps there directly. **No symlink required** — the old `~/.agent/diagrams/brand/` symlink bridge is gone (see migration note below).

### Image discovery

**Deployed (Cloudflare Pages)**: galleries discover images via static `manifest.json` files in each batch directory. These are generated by `gen-image-manifests.py` as part of `make forge deploy`. The gallery fetches `<batch-dir>/manifest.json` at load time.
...
Older instructions referenced `~/.agent/diagrams/` with a symlink to `~/projects/lyra/brand/`. Both are deprecated:
- Canonical brand root moved: `~/projects/lyra/brand/` → `~/.roxabi/forge/lyra/brand/`
- Server moved: `~/.agent/diagrams/serve.py` → `~/.roxabi/forge/serve.py` (deployed by `make -C ~/projects/roxabi-forge/plugins/forge deploy`)
- Deploy target renamed: `make diagrams deploy` → `make forge deploy`

No manual action is needed if the forge plugin is registered (`make forge install` from `~/projects/`).
...
```
http://localhost:8080/lyra/brand/avatar-gallery.html
```
...
> **For new galleries, use the `forge-gallery` skill (v0.4.0+).** It handles BATCHES discovery, pivot grouping (column/row), filtering, sorting, search, size controls, starring, and lightbox — no hand-rolled HTML required. Data-driven galleries load `face-scores.json` for score/cluster-based views.
>
> The instructions below describe the **hand-coded pattern** used by the existing `avatar-gallery.html` (which pre-dates the skill). Use this only when extending that specific gallery, or for a custom layout the skill can't express. For everything else, reach for `forge-gallery` first.
...
1. Generate images to `~/.roxabi/forge/lyra/brand/concepts/avatar-lyra-v<N>/`
...
```bash
# Create prompt file
mkdir -p ~/.roxabi/forge/lyra/brand/prompts/avatar-final/
...
```bash
cd ~/projects/imageCLI
uv run imagecli batch ~/.roxabi/forge/lyra/brand/prompts/avatar-final/ \
  --output-dir ~/.roxabi/forge/lyra/brand/concepts/avatar-final/
```
...
~/.roxabi/forge/
  serve.py                   # Forge dev server (port 8080, SSE live-reload)
  Makefile                   # Runtime Makefile — build, deploy, start/stop
```
...
```bash
# From ~/projects/
make forge ps      # is the server running?
make forge start   # start it
make forge logs    # tail stdout
```
